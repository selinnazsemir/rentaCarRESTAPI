...
namespace DataAccessLayer.Repositories
{
    public class AraclarRepository : IAraclarRepository
    {
        ...
        public DataTable GelirGider()
        {
            DataTable dt = null;
            string strSql = @"EXEC GelirGiderler";
            dt = Baglanti.VeriGetir(strSql);
            return dt;
        }
        ...
        public bool KiralananıSal(int id)
        {
            ...
            {
                Baglanti.BaglantiKontrol();
                string strSql = @"UPDATE Araclar SET KiralandiMi = 0 WHERE AracId = '" + id + "'";
                Baglanti.VeriGuncelle(strSql);
                return true;
            }
            catch (Exception ex) { }
        ...
        public DataTable KiralananlariGetir()
        {
            DataTable dt = null;
            string strSql = @"SELECT * FROM Araclar WHERE KiralandiMi= 1 AND SilindiMi=0 AND RezerveMi = 0";
            dt = Baglanti.VeriGetir(strSql);
        ...
        public DataTable ToplamGelirGider()
        {
            DataTable dt = null;
            string strSql = @"EXEC ToplamGelirGiderler";
            dt = Baglanti.VeriGetir(strSql);
            return dt;
        }

        public DataTable Ortalama(string baslangicTarihi)
        {
            DataTable dt = null;
            string strSql = @"EXEC OrtalamakmGunTariheGore '" + baslangicTarihi + "'";
            dt = Baglanti.VeriGetir(strSql);
            return dt;
        }
        ...
        public DataTable VergiHesapla()
        {
            DataTable dt = null;
            string strSql = @"SELECT AracMarka,AracModel,AracGunlukFiyat,Vergi FROM Araclar WHERE  SilindiMi=0 AND RezerveMi = 0";
            dt = Baglanti.VeriGetir(strSql);
            return dt;
        }
